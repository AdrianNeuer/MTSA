...
class MLTrainer:
    ...
    def train(self, seq_len=96, pred_len=32):
    ...
    def test(self, dataset, seq_len=96, pred_len=32):
        if dataset.type == 'm4':
            ...
            subseries = np.concatenate(
                ([sliding_window_view(v, seq_len + pred_len) for v in dataset.test_data]))
    ...
    def evaluate(self, dataset, seq_len=96, pred_len=32):
        if dataset.type == 'm4':
            ...
            subseries = np.concatenate(
                ([sliding_window_view(v, seq_len + pred_len) for v in test_data]))
            test_X = subseries[:, :seq_len]
