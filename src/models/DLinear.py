...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        seasonal, trend = seasonal.permute(0, 2, 1).to(
            device), trend.permute(0, 2, 1).to(device)
        if self.individual:
            seasonal_output = torch.zeros([seasonal.size(0), seasonal.size(
                1), self.pred_len], dtype=seasonal.dtype).to(seasonal.device)
            trend_output = torch.zeros([trend.size(0), trend.size(
                1), self.pred_len], dtype=trend.dtype).to(trend.device)
            for i in range(self.channels):
